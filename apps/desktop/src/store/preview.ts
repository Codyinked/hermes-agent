 ...
 *
 *  A build that predates scoping stored ONE global array under this key. That
 *  value is deliberately ABANDONED rather than migrated: it records nothing
 *  about which profile wrote it, so handing it to whichever profile happens to
 *  ask first puts one agent's tabs into another agent's rail — exactly the
 *  cross-agent bleed this scoping exists to prevent. Losing a few preview tabs
 *  once is recoverable by reopening them; misattributing them is a bug the user
 *  cannot explain. */
